...
pub fn get_w_cost() -> f64 {
    unsafe {
        get_w_cost_mpc_controller() as f64
    }
}
...
pub fn grad(u: &[f64], xi: &[f64], static_params: &[f64], cost_jacobian: &mut [f64]) -> i32 {
    ...
    unsafe {
        grad_cost_function_mpc_controller(
            arguments.as_ptr(),
            grad.as_mut_ptr()
        ) as i32
    }
...
pub fn mapping_f1(
    ...
    unsafe {
         mapping_f1_function_mpc_controller(
            arguments.as_ptr(),
...
pub fn mapping_f2(
    ...
    unsafe {
         mapping_f2_function_mpc_controller(
            arguments.as_ptr(),
            constraints.as_mut_ptr()
        ) as i32
    }
...
pub fn precondition(
    ...
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    unsafe {
         preconditioning_www_mpc_controller(arguments.as_ptr()) as i32
    }
}
...
pub fn initial_penalty(
    ...
    // preconditioning_init_penalty_function_mpc_controller
    let arguments = &[u.as_ptr(), static_params.as_ptr()];
    let ip = &mut [rho_init as *mut c_double];

    unsafe {
         init_penalty_function_mpc_controller(
            arguments.as_ptr(),
            ip.as_mut_ptr(),
        ) as i32
    }

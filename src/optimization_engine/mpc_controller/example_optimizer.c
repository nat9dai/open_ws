 ...
 * gcc -Wall -std=c99 -pedantic \
    example_optimizer.c -l:libmpc_controller.a \
...
#include <stdio.h>
#include "mpc_controller_bindings.h"
...
int main() {
    ...
    double y[MPC_CONTROLLER_N1] = {0.0};

    /* obtain cache           */
    mpc_controllerCache *cache = mpc_controller_new();

    /* solve                  */
    mpc_controllerSolverStatus status = mpc_controller_solve(cache, u, p, y, &init_penalty);
    ...
    for (i = 0; i < MPC_CONTROLLER_NUM_DECISION_VARIABLES; ++i) {
        printf("u[%d] = %g\n", i, u[i]);
    }

    printf("\n");
    for (i = 0; i < MPC_CONTROLLER_N1; ++i) {
        printf("y[%d] = %g\n", i, status.lagrange[i]);
    }
